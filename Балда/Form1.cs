...
namespace балд
{
    public partial class Form2 : Form
    {
        ...
        bool lang = true, player = true, inp = true;//Логические переменные для смены языка, игрока и регулировки ввода
        string filename2 = "dicru.txt"/*, filename1 = "dicen.txt", filename3 = "records.txt"*/, rec = "";//Имена файлов со словарями и рекордами
        ...
        void maincheck(string file, string[] xx, ref string x, string rts, bool lang)
        {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (selectij)//Если правильно выделено...
            {
                if (startwinp)//Если буква использована
                {
                    ...
                    if (lang)//входим в проверку, если на русском, то...
                        maincheck(filename2, xx, ref x, rts, lang);
                    else
                        maincheck(filename2, xx, ref x, rts, lang);
                    ...
                    if (winner)//Если пустых мест нет
                    {
                        ...
                        FileStream ff = new FileStream(filename2, FileMode.OpenOrCreate, FileAccess.ReadWrite);
                        StreamReader rr = new StreamReader(ff);
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
                        ...
                        if (e.RowIndex == 0 && e.ColumnIndex == 0)
                        {
                            ...
                            if (MyCell1 != " " || MyCell2 != " ")
                            {
                                if (lang)
                                {
                                    ...
                                    if ((int)myCellValue.Substring(0, 1).ToLower()[0] >= 97 && (int)myCellValue.Substring(0, 1).ToLower()[0] <= 122)
                                    {
                                        myGrid.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = myCellValue.Substring(0, 1).ToLower();
                                        pastedm(ref B, myGrid);
                                        if (e.RowIndex != 2)
            ...
            //    myGrid.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = B[e.RowIndex][e.ColumnIndex];
                ii = e.RowIndex;
            jj = e.ColumnIndex;
        }
        ...
        {
            if (textBox1.Text == "")
                MessageBox.Show("Введите имя 1 - го играка!!!", "Ошибка", MessageBoxButtons.OK);
            else
                if (textBox2.Text == "")
                    MessageBox.Show("Введите имя 2 - го играка!!!", "Ошибка", MessageBoxButtons.OK);
            else
            {
